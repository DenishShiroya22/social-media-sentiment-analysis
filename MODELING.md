...
## Post-benchmark experimental research

After the completed single official TEST evaluation, a separate track examines whether the frozen LR and LinearSVC development pipelines combine usefully, and whether sparse multiclass XGBoost helps. It does not revisit TEST. All new model selection is three-fold stratified TRAIN CV; each selected setting is fitted on full TRAIN and compared once on VALIDATION. The original final model, TEST metrics, manifest, summary and model card remain historical records.

Two-estimator hard voting resolves disagreements to LR, which makes its output exactly the frozen LR prediction. Soft voting uses LinearSVC sigmoid probabilities calibrated inside TRAIN folds, then chooses among five weights by TRAIN CV. Stacking uses inner out-of-fold LR probabilities and SVM decision scores (six features) with a small logistic meta-model, evaluated by outer TRAIN CV. XGBoost fits combined word (1,2) plus character (3,5) TF-IDF within each fold and keeps the matrix CSR sparse; ten constrained tree/weight configurations are scored by macro-F1. Balanced multiclass row weights, when used, come from fold-training labels only. VALIDATION is never part of CV or calibration.

The [experimental summary](reports/experimental/ensemble_summary.md), [comparison table](reports/experimental/ensemble_comparison.csv), [disagreement analysis](reports/experimental/model_disagreement.md), [paired bootstrap](reports/experimental/bootstrap_comparison.md), [XGBoost search](reports/experimental/xgboost_results.csv), and [notebook](notebooks/05_ensemble_experiments.ipynb) contain the resulting development evidence. All new artifacts are isolated under models/experimental and labeled EXPERIMENTAL DEVELOPMENT MODEL. A promising result requires a newly collected, manually labeled external brand-comment dataset; the official TweetEval TEST set is closed.
